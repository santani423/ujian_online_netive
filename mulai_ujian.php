<?php
// mulai_ujian.php - Untuk siswa mengerjakan ujian (DENGAN MULTI KELAS SUPPORT)
...
// AMBIL DATA UJIAN - DENGAN MULTI KELAS SUPPORT (FIND_IN_SET)
$sql_ujian = "SELECT u.*, mp.nama_mapel, mp.kode_mapel,
              ...
              AND NOW() BETWEEN u.waktu_mulai AND u.waktu_selesai";
              
$stmt_ujian = $pdo->prepare($sql_ujian);
$stmt_ujian->execute([$ujian_id, $siswa['kelas']]);
$ujian = $stmt_ujian->fetch();

if (!$ujian) {
    flashMessage('danger', 'Ujian tidak ditemukan atau tidak dapat diakses!');
    header('Location: daftar_ujian.php');
    exit();
}

// CEK APAKAH SISWA SUDAH MEMULAI UJIAN INI
$stmt_hasil = $pdo->prepare("SELECT * FROM hasil_ujian WHERE ujian_id = ? AND siswa_id = ?");
$stmt_hasil->execute([$ujian_id, $siswa['id']]);
$hasil_ujian = $stmt_hasil->fetch();
...
if ($hasil_ujian) {
    ...
    if ($hasil_ujian['status'] == 'selesai') {
        flashMessage('info', 'Anda sudah menyelesaikan ujian ini!');
        header('Location: detail_hasil_siswa.php?id=' . $hasil_ujian['id']);
        exit();
    }
    // Gunakan waktu_mulai dari database
    $waktu_mulai_pengerjaan = strtotime($hasil_ujian['waktu_mulai']);
} else {
    try {
        $waktu_sekarang = waktuSekarang();
        $stmt_insert = $pdo->prepare("INSERT INTO hasil_ujian (ujian_id, siswa_id, waktu_mulai, status) VALUES (?, ?, ?, 'sedang_ujian')");
        $stmt_insert->execute([$ujian_id, $siswa['id'], $waktu_sekarang]);
        $hasil_ujian_id = $pdo->lastInsertId();
        $waktu_mulai_pengerjaan = strtotime($waktu_sekarang);
    } catch (Exception $e) {
        flashMessage('danger', 'Gagal memulai ujian: ' . $e->getMessage());
        header('Location: daftar_ujian.php');
        exit();
    }
}
...
// CEK WAKTU UJIAN - MENGGUNAKAN waktu_selesai DARI DATABASE
$waktu_sekarang = time();
$waktu_selesai_ujian_db = strtotime($ujian['waktu_selesai']);
...
if ($sisa_waktu <= 0) {
    $stmt_selesai = $pdo->prepare("UPDATE hasil_ujian SET status = 'selesai', waktu_selesai = ? WHERE id = ?");
    $stmt_selesai->execute([date('Y-m-d H:i:s', $waktu_sekarang), $hasil_ujian_id]);
    flashMessage('warning', 'Waktu ujian telah habis!');
...
function deteksiJenisSoal($soal) {
    ...
    foreach ($lines_a as $line) {
        if (preg_match('/^(\d+|[a-z])[\.\)\s]/', trim($line))) {
            $has_numbered_a = true;
            break;
        }
    }
...
$title = "Kerjakan Ujian - " . $ujian['judul_ujian'];
?>
...
<body>
    ...
    <div id="penaltyOverlay" class="penalty-overlay d-none">
        <div class="container">
            ...
            <p class="lead mb-4">Anda terdeteksi melakukan pelanggaran selama ujian berlangsung.</p>
            <div class="alert alert-danger" style="max-width: 600px; margin: 0 auto;">
                ...
                <p>Anda telah keluar dari halaman ujian atau membuka tab lain selama ujian berlangsung.</p>
                <p class="mb-0">Sebagai konsekuensi, ujian Anda ditunda selama:</p>
            </div>
    ...
    <div id="fullscreenWarning" class="fullscreen-warning d-none">
        <div class="container">
            ...
            <h2>Peringatan!</h2>
            <p class="lead">Jangan keluar dari halaman ujian selama ujian berlangsung.</p>
            <button class="btn btn-warning btn-lg mt-3" onclick="hideFullscreenWarning()">
                <i class="fas fa-check me-2"></i>Mengerti, Lanjutkan Ujian
            </button>
        </div>
    </div>
                ...
                <div class="timer-container">
                    ...
                    <div class="card mt-3">
                        <div class="card-body">
                            ...
                            <p class="card-text small mb-1"><strong>Mata Pelajaran:</strong><br><?= htmlspecialchars($ujian['nama_mapel']) ?></p>
                            <p class="card-text small mb-1"><strong>Judul Ujian:</strong><br><?= htmlspecialchars($ujian['judul_ujian']) ?></p>
                            <p class="card-text small mb-1"><strong>Jumlah Soal:</strong><br><?= count($soal_list) ?> Soal</p>
                            <p class="card-text small"><strong>Durasi:</strong><br><?= $ujian['durasi'] ?> Menit</p>
                            
                            <?php if ($random_setting == 1): ?>
                            <div class="info-acak small mt-2"><i class="fas fa-random me-1"></i><strong>Urutan soal diacak</strong> untuk setiap peserta</div>
                            <?php endif; ?>
            ...
            <div class="col-lg-9">
                <div class="card soal-card">
                    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">Kerjakan Ujian</h5>
    ...
    <script>
        ...
        function autoSaveJawaban() {
            ...
            fetch('mulai_ujian.php?id=<?= $ujian_id ?>', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ auto_save: 1, jawaban: jawabanData })
            }).then(response => response.json()).then(data => { if (data.success) showSaveIndicator('Auto-saved'); })
        ...
        function tandaiRaguRagu() { 
            if (!isPenaltyActive) { 
                const navBtn = document.getElementById(`navSoal${currentSoal}`); 
                if (navBtn) navBtn.classList.toggle('ragu-ragu'); 
            } 
        }
        ...
        function selesaikanUjianOtomatis() { 
            fetch('mulai_ujian.php?id=<?= $ujian_id ?>', { 
                method: 'POST', 
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, 
                body: 'selesai_ujian=1' 
            }).then(() => window.location.href = 'detail_hasil_siswa.php?id=<?= $hasil_ujian_id ?>&timeout=1'); 
        }
        ...
        document.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'ArrowRight' && !e.ctrlKey && !e.altKey) { 
                ...
                if (target < totalSoal) showSoal(target); 
                e.preventDefault();
            }
        });
        ...
        function addWarning() {
            ...
            if (warningCount >= 2) activatePenalty();
        }
        
        function updateWarningDisplay() {
            const warningElement = document.getElementById('warningCounter');
        ...
        function activatePenalty() {
            ...
            logKecurangan('penalty_activated', 'Penalti 5 menit diaktifkan karena pelanggaran berulang');
        }
        
        function updatePenaltyDisplay() { 
            document.getElementById('penaltyTimer').textContent = `${Math.floor(penaltyTimeLeft/60).toString().padStart(2,'0')}:${(penaltyTimeLeft%60).toString().padStart(2,'0')}`; 
        }
        ...
        function endPenalty() {
            ...
            warningCount = 0; 
            updateWarningDisplay();
            document.getElementById('penaltyOverlay').classList.add('d-none');
            startTimer(); 
            startAutoSave();
        }
        ...
        document.addEventListener('contextmenu', e => { 
            e.preventDefault(); 
            logKecurangan('right_click', 'Menggunakan klik kanan'); 
            addWarning(); 
            return false; 
        });
