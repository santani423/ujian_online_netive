<?php
// kelola_soal.php - KELAS BISA LEBIH DARI SATU (MULTIPLE SELECT DENGAN CTRL+KLIK)
...
$soal_hari_ini = $pdo->query("SELECT COUNT(*) FROM soal WHERE DATE(created_at) = CURDATE()")->fetchColumn();
$total_skor = $pdo->query("SELECT COALESCE(SUM(skor), 0) FROM soal")->fetchColumn();
$soal_dengan_gambar = $pdo->query("SELECT COUNT(*) FROM soal WHERE gambar_soal IS NOT NULL AND gambar_soal != ''")->fetchColumn();
$soal_dengan_video = $pdo->query("SELECT COUNT(*) FROM soal WHERE video_soal IS NOT NULL AND video_soal != ''")->fetchColumn();

// Hitung statistik per jenis soal
...
$all_soal = $pdo->query("SELECT * FROM soal")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <style>
        ...
        /* Style untuk modal import yang rapi */
        .modal-import-body {
            max-height: 60vh;
            overflow-y: auto;
            padding: 15px;
...
<div class="container-fluid p-4">
    ...
    if (isset($_SESSION['import_errors'])) {
        ...
        echo '<ul class="mb-0" style="max-height: 200px; overflow-y: auto;">';
    ...
    <div class="row mb-4 g-3">
        ...
        <div class="col-md-2 col-sm-4 col-6">
            <div class="card border-0 text-white h-100" style="background-color: #6f42c1 !important;">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="col-lg-3 mb-4">
            <div class="card mb-4">
                ...
                <div class="card-body p-0">
                    <div class="list-group list-group-flush">
                        <a href="kelola_mapel.php" class="list-group-item list-group-item-action d-flex align-items-center">
                            <i class="fas fa-cog text-primary me-3"></i>
                            <div><div class="fw-bold">Kelola Mapel</div><small class="text-muted">Tambah/edit mata pelajaran</small></div>
                        </a>
                        <a href="download_template_soal.php" class="list-group-item list-group-item-action d-flex align-items-center">
                            <i class="fas fa-download text-success me-3"></i>
                            <div><div class="fw-bold">Download Template</div><small class="text-muted">Template import Excel</small></div>
                        </a>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header bg-light d-flex justify-content-between align-items-center">
                    <h6 class="mb-0"><i class="fas fa-users me-2"></i>Daftar Kelas</h6>
                    <span class="badge bg-primary"><?= count($kelas_list) ?></span>
                </div>
                <div class="card-body p-0">
                    <div style="max-height: 300px; overflow-y: auto;">
                        <table class="table table-sm table-borderless mb-0">
                            <thead class="table-light"><tr><th class="ps-3"><small>Kelas</small></th><th class="text-end pe-3"><small>Jumlah Soal</small></th></tr></thead>
                            <tbody>
                                <?php foreach ($kelas_list as $kelas): 
                                    $stmt = $pdo->prepare("SELECT COUNT(*) FROM soal WHERE FIND_IN_SET(?, kelas) OR kelas = ?");
                                    $stmt->execute([$kelas['kelas'], $kelas['kelas']]);
                                    $jumlah = $stmt->fetchColumn();
                                ?>
                                <tr><td class="ps-3"><small><span class='badge bg-info'><?= $kelas['kelas'] ?></span></small></td>
                                    <td class="text-end pe-3"><small><span class='badge bg-secondary'><?= $jumlah ?></span></small></td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
        ...
        <div class="col-lg-9">
            ...
            <div class="card mb-4">
                <div class="card-header bg-light d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <a href="download_resume_soal_pdf.php<?= !empty($_GET) ? '?' . http_build_query($_GET) : '' ?>" class="btn btn-danger btn-sm" target="_blank">
                    ...
                    <form method="GET" class="row g-3 align-items-end">
                        <div class="col-md-3">
                            ...
                            <select class="form-select" name="mapel_id">
                                ...
                                while ($mapel = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                    $selected = (isset($_GET['mapel_id']) && $_GET['mapel_id'] == $mapel['id']) ? 'selected' : '';
                                    echo "<option value='{$mapel['id']}' $selected>{$mapel['kode_mapel']} - {$mapel['nama_mapel']}</option>";
                                }
                                ?>
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <select class="form-select" name="kelas">
                                ...
                                <?php foreach ($kelas_list as $kelas): ?>
                                <option value="<?= $kelas['kelas'] ?>" <?= (isset($_GET['kelas']) && $_GET['kelas'] == $kelas['kelas']) ? 'selected' : '' ?>><?= htmlspecialchars($kelas['kelas']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <select class="form-select" name="jenis_soal">
                                ...
                                <option value="pilihan_ganda" <?= (isset($_GET['jenis_soal']) && $_GET['jenis_soal'] == 'pilihan_ganda') ? 'selected' : '' ?>>Pilihan Ganda (A-E)</option>
                                <option value="pilihan_ganda_kompleks" <?= (isset($_GET['jenis_soal']) && $_GET['jenis_soal'] == 'pilihan_ganda_kompleks') ? 'selected' : '' ?>>Pilihan Ganda Kompleks</option>
                                <option value="essay" <?= (isset($_GET['jenis_soal']) && $_GET['jenis_soal'] == 'essay') ? 'selected' : '' ?>>Essay</option>
                                <option value="menjodohkan" <?= (isset($_GET['jenis_soal']) && $_GET['jenis_soal'] == 'menjodohkan') ? 'selected' : '' ?>>Menjodohkan</option>
                                <option value="benar_salah" <?= (isset($_GET['jenis_soal']) && $_GET['jenis_soal'] == 'benar_salah') ? 'selected' : '' ?>>Benar/Salah</option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <select class="form-select" name="media_type">
                                ...
                                <option value="gambar" <?= (isset($_GET['media_type']) && $_GET['media_type'] == 'gambar') ? 'selected' : '' ?>>Dengan Gambar</option>
                                <option value="video" <?= (isset($_GET['media_type']) && $_GET['media_type'] == 'video') ? 'selected' : '' ?>>Dengan Video</option>
                                <option value="tanpa_media" <?= (isset($_GET['media_type']) && $_GET['media_type'] == 'tanpa_media') ? 'selected' : '' ?>>Tanpa Media</option>
                            </select>
                        </div>
                        <div class="col-md-12 mt-3">
                            <button type="submit" class="btn btn-primary me-2"><i class="fas fa-filter me-2"></i>Filter</button>
                            <a href="kelola_soal.php" class="btn btn-outline-secondary"><i class="fas fa-refresh me-2"></i>Reset</a>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <!-- Tabel Soal -->
            <div class="card">
                <div class="card-header bg-light"><h6 class="mb-0"><i class="fas fa-table me-2"></i>Daftar Soal</h6></div>
                <div class="card-body p-0">
                        ...
                        <table class="table table-hover table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)): 
                                    ...
                                    if (isset($_GET['jenis_soal']) && !empty($_GET['jenis_soal']) && $jenis_soal != $_GET['jenis_soal']) continue;
                                    ...
                                    $has_gambar = !empty($row['gambar_soal']);
                                    $has_video = !empty($row['video_soal']);
                                    
                                    $badge_class = '';
                                    $jenis_text = '';
                                    switch($jenis_soal) {
                                        case 'pilihan_ganda': $badge_class = 'bg-primary'; $jenis_text = 'PG (A-E)'; break;
                                        case 'pilihan_ganda_kompleks': $badge_class = 'bg-purple'; $jenis_text = 'PG Kompleks'; break;
                                        case 'essay': $badge_class = 'bg-info'; $jenis_text = 'Essay'; break;
                                        case 'menjodohkan': $badge_class = 'bg-warning'; $jenis_text = 'Jodoh'; break;
                                        case 'benar_salah': $badge_class = 'bg-danger'; $jenis_text = 'B/S'; break;
                                        default: $badge_class = 'bg-secondary'; $jenis_text = '?';
                                    }
                                    ...
                                    if ($jenis_soal == 'pilihan_ganda') {
                                        ...
                                        $jwb = json_decode($row['jawaban_kompleks'] ?? '[]', true);
                                        $labels = [];
                                        if (is_array($jwb)) foreach ($jwb as $j) $labels[] = '<span class="badge bg-success me-1">' . strtoupper($j) . '</span>';
                                        $jawaban_display = '<div class="mb-1">' . implode(' ', $labels) . '</div><small class="text-muted">' . ($row['skor_per_jawaban'] ?? 0) . ' poin/jawaban</small>';
                                    } elseif ($jenis_soal == 'essay') {
                                        $jawaban_display = '<span class="badge bg-info">Essay</span><br><small>Manual</small>';
                                    } elseif ($jenis_soal == 'menjodohkan') {
                                        $jawaban_display = '<span class="badge bg-warning">Jodoh</span>';
                                    } elseif ($jenis_soal == 'benar_salah') {
                                        $bs = ($row['jawaban_benar'] == 'a') ? 'Benar' : 'Salah';
                                        $jawaban_display = '<span class="badge ' . ($row['jawaban_benar'] == 'a' ? 'bg-success' : 'bg-danger') . '">' . $bs . '</span>';
                                    }
                                ...
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <div class="fw-bold text-dark mb-1"><?= htmlspecialchars($pertanyaan_pendek) ?></div>
                                        <?php if ($has_gambar || $has_video): ?>
                                            <div class="d-flex gap-1 mb-1">
                                                <?php if ($has_gambar): ?><span class="badge bg-success"><i class="fas fa-image me-1"></i>Gambar</span><?php endif; ?>
                                                <?php if ($has_video): ?><span class="badge bg-warning"><i class="fas fa-video me-1"></i>Video</span><?php endif; ?>
                                            </div>
                                        <?php endif; ?>
                                    </td>
                                    <td class="text-center align-middle"><span class="badge <?= $badge_class ?>"><?= $jenis_text ?></span></td>
...
<!-- MODAL IMPORT SOAL (DIPERBAIKI - TIDAK MELEBIHI BATAS) -->
<div class="modal fade" id="importSoalModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-scrollable" style="max-width: 700px;">
        <div class="modal-content">
            ...
            <form action="proses_import_soal.php" method="POST" enctype="multipart/form-data">
                <div class="modal-body" style="max-height: 65vh; overflow-y: auto;">
                    ...
                    <div class="card border-success mb-3">
                        ...
                        <div class="card-body text-center py-2">
                            <a href="download_template_soal.php" class="btn btn-outline-success btn-sm">
                    ...
                    <div class="alert alert-info p-2 mb-0">
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered mb-0 template-table">
                                <thead>
                                    <tr>
                                        <th>jenis_soal</th><th>mapel_id</th><th>kelas</th><th>pertanyaan</th><th>opsi_a</th><th>opsi_b</th><th>opsi_c</th><th>opsi_d</th><th>opsi_e</th><th>jawaban_benar</th><th>skor</th>
                                    </tr>
                                </thead>
                                <tbody>
...
<!-- MODAL TAMBAH/EDIT SOAL (SATU MODAL) -->
<div class="modal fade" id="soalModal" tabindex="-1" data-bs-backdrop="static">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form action="proses_soal.php" method="POST" enctype="multipart/form-data" id="formSoal">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" name="mapel_id" id="mapel_id" required>
                                        ...
                                        <?php $stmt = $pdo->query("SELECT * FROM mata_pelajaran ORDER BY nama_mapel");
                                        while ($mapel = $stmt->fetch(PDO::FETCH_ASSOC)) echo "<option value='{$mapel['id']}'>{$mapel['kode_mapel']} - {$mapel['nama_mapel']}</option>"; ?>
                                    </select>
                                </div>
                                <div class="col-md-6 mb-3">
...
<!-- MODAL DETAIL SOAL -->
<div class="modal fade" id="detailSoalModal" tabindex="-1" data-bs-backdrop="static">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
...
function hapusSoal(id) {
    if (confirm('Apakah Anda yakin ingin menghapus soal ini?')) {
        window.location.href = 'proses_soal.php?action=hapus&id=' + id;
    }
}
...
// Fungsi insert simbol ke textarea
function insertSymbolToTextarea(symbol, textareaId) {
    let targetTextarea = null;
    if (textareaId) {
        targetTextarea = document.getElementById(textareaId);
    } else {
        targetTextarea = document.activeElement;
        if (!targetTextarea || targetTextarea.tagName !== 'TEXTAREA') {
            targetTextarea = document.querySelector('#dynamicFormContainer textarea');
        }
    }
    ...
    if (targetTextarea && targetTextarea.tagName === 'TEXTAREA') {
        ...
        targetTextarea.selectionStart = start + symbol.length;
        targetTextarea.selectionEnd = start + symbol.length;
        const event = new Event('input', { bubbles: true });
        targetTextarea.dispatchEvent(event);
    } else {
        alert('Klik pada kolom teks terlebih dahulu sebelum memasukkan simbol!');
    }
}

// Fungsi membuat dropdown menu simbol
function createSymbolDropdown(textareaId) {
    ...
    for (const [category, symbols] of Object.entries(mathSymbols)) {
        ...
        symbols.forEach(symbol => {
            ...
            link.onclick = (function(s, id) {
                return function(e) {
                    e.preventDefault();
                    insertSymbolToTextarea(s, id);
                };
            })(symbol, textareaId);
...
// Fungsi wrap text untuk formatting
...
// Fungsi membuat toolbar editor lengkap
function createFullEditorToolbar(textareaId) {
    ...
    textGroup.innerHTML = `
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('**', '**', '${textareaId}')" title="Bold"><b>B</b></button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('*', '*', '${textareaId}')" title="Italic"><i>I</i></button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('__', '__', '${textareaId}')" title="Underline"><u>U</u></button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('~~', '~~', '${textareaId}')" title="Strikethrough"><s>S</s></button>
    `;
    ...
    headingGroup.innerHTML = `
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('# ', '', '${textareaId}')" title="Heading 1">H1</button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('## ', '', '${textareaId}')" title="Heading 2">H2</button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('### ', '', '${textareaId}')" title="Heading 3">H3</button>
    `;
    ...
    listGroup.innerHTML = `
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('- ', '', '${textareaId}')" title="Bullet List"><i class="fas fa-list-ul"></i></button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('1. ', '', '${textareaId}')" title="Numbered List"><i class="fas fa-list-ol"></i></button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('- [ ] ', '', '${textareaId}')" title="Checklist"><i class="fas fa-check-square"></i></button>
    `;
    ...
    latexGroup.innerHTML = `
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('$', '$', '${textareaId}')" title="Inline Math (LaTeX)"><i class="fas fa-square-root-alt"></i> $</button>
        <button type="button" class="btn-toolbar-icon" onclick="wrapText('$$\\n', '\\n$$', '${textareaId}')" title="Display Math (LaTeX)"><i class="fas fa-square-root-alt"></i> $$</button>
    `;
...
function loadDynamicForm(jenis) {
    ...
    if (jenis === 'pilihan_ganda') {
        container.innerHTML = `
            <div class="card mb-3">
                <div class="card-header bg-light">Pertanyaan & Media</div>
                <div class="card-body">
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6"><label>Gambar Soal</label><input type="file" class="form-control" name="gambar_soal" accept="image/*" id="gambar_soal"><div id="gambarPreview"></div></div>
                        <div class="col-md-6"><label>Video Soal</label><input type="text" class="form-control" name="video_soal" id="video_soal" placeholder="URL YouTube"></div>
                    </div>
                </div>
            </div>
            <div class="card mb-3">
                <div class="card-header bg-light">Opsi Jawaban (A-E)</div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-2"><label>A <span class="text-danger">*</span></label><textarea class="form-control" name="opsi_a" id="txt_opsi_a" rows="2" required></textarea></div>
            ...
            <input type="hidden" name="jawaban_kompleks" value="">
            <input type="hidden" name="skor_per_jawaban" value="0">
        ...
        container.innerHTML = `
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6"><label>Gambar Soal</label><input type="file" class="form-control" name="gambar_soal" accept="image/*"></div>
                        <div class="col-md-6"><label>Video Soal</label><input type="text" class="form-control" name="video_soal" placeholder="URL YouTube"></div>
                    </div>
                </div>
            </div>
            <div class="card mb-3">
                <div class="card-header bg-light">Opsi Jawaban (Centang yang benar)</div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-2"><label>A</label><textarea class="form-control" name="opsi_a" id="txt_opsi_a" rows="2"></textarea></div>
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <div class="border rounded p-2">
                                <label class="me-3"><input type="checkbox" name="jawaban_kompleks_checkbox[]" value="a" id="cek_a"> A</label>
                                ...
                                <label class="me-3"><input type="checkbox" name="jawaban_kompleks_checkbox[]" value="e" id="cek_e"> E</label>
                            </div>
                        </div>
                    </div>
        ...
        container.innerHTML = `
            <div class="card mb-3">
                <div class="card-header bg-light">Pertanyaan Essay</div>
                <div class="card-body">
                    <label class="form-label fw-bold">Pertanyaan <span class="text-danger">*</span></label>
        ...
        container.innerHTML = `
            <div class="card mb-3">
                <div class="card-header bg-light">Pertanyaan Menjodohkan</div>
                <div class="card-body">
        ...
        container.innerHTML = `
            <div class="card mb-3">
                <div class="card-header bg-light">Pernyataan Benar/Salah</div>
                <div class="card-body">
                    ...
                    <div class="row mt-3">
                        <div class="col-md-6"><label>Opsi A</label><input type="text" class="form-control" name="opsi_a" value="Benar" readonly></div>
                        <div class="col-md-6"><label>Opsi B</label><input type="text" class="form-control" name="opsi_b" value="Salah" readonly></div>
                    </div>
            ...
            if (data.success) {
                ...
                if (jenis === 'pilihan_ganda') {
                    ...
                    setTimeout(() => {
                        ...
                        document.getElementById('skor_per_jawaban').dispatchEvent(event);
                    }, 100);
                }
                else if (jenis === 'essay') {
                    document.getElementById('txt_opsi_a').value = data.soal.opsi_a;
...
function detailSoal(id, jenis) {
    console.log('Detail soal ID:', id, 'Jenis:', jenis);
    ...
    fetch('ajax_detail_soal.php?id=' + id + '&jenis=' + jenis)
        ...
        .catch(error => {
            ...
            document.getElementById('detailSoalContent').innerHTML = '<div class="alert alert-danger">Gagal memuat detail soal. Silakan coba lagi.</div>';
        });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    radios.forEach(radio => {
        radio.addEventListener('change', function() {
            if (this.checked) loadDynamicForm(this.value);
        });
    });
        ...
        formSoal.addEventListener('submit', function(e) {
            ...
            if (kelasSelect) {
                ...
                if (selectedOptions.length > 0) {
                    // Hapus name="kelas[]" dan ganti dengan hidden input
                    kelasSelect.name = '';
